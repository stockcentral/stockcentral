...
export default function RMA() {
  ...
  const loadStatusColors = async () => {
    try {
      const r = await api.get('/settings/general');
      if (r.data?.rma_status_colors) {
        setStatusColors({...DEFAULT_STATUS_COLORS, ...JSON.parse(r.data.rma_status_colors)});
      }
    } catch(e) {}
  };

  const SC = (s) => statusColors[s] || '#6b7280';
