...
{tab==='general'&&(<div style={{maxWidth:580}}><h2 style={{fontSize:22,fontWeight:700,marginBottom:8}}>General Settings</h2><p style={{opacity:.5,fontSize:13,marginBottom:32}}>Configure cost calculation and Shopify sync behavior.</p><div style={{marginBottom:24,padding:'20px 24px',background:'rgba(255,255,255,.04)',borderRadius:12,border:'1px solid rgba(255,255,255,.08)'}}><label style={{display:'block',fontSize:14,fontWeight:600,marginBottom:6}}>Cost Update Mode</label><p style={{fontSize:12,opacity:.5,marginBottom:10}}>Should StockCentral update product cost automatically when purchase orders are received?</p><select value={general.cost_update_mode} onChange={e=>setGeneral(g=>({...g,cost_update_mode:e.target.value}))} style={{width:'100%',padding:'10px 12px',borderRadius:8,border:'1px solid rgba(255,255,255,.15)',background:'rgba(255,255,255,.06)',color:'inherit',fontSize:14}}><option value="auto">Auto — update cost when PO items are received</option><option value="manual">Manual — update cost manually only</option></select></div><div style={{marginBottom:24,padding:'20px 24px',background:'rgba(255,255,255,.04)',borderRadius:12,border:'1px solid rgba(255,255,255,.08)'}}><label style={{display:'block',fontSize:14,fontWeight:600,marginBottom:6}}>Cost Calculation Method</label><p style={{fontSize:12,opacity:.5,marginBottom:10}}>How should StockCentral calculate the cost of each item?</p><select value={general.cost_calculation_method} onChange={e=>setGeneral(g=>({...g,cost_calculation_method:e.target.value}))} style={{width:'100%',padding:'10px 12px',borderRadius:8,border:'1px solid rgba(255,255,255,.15)',background:'rgba(255,255,255,.06)',color:'inherit',fontSize:14}}><option value="1">Most recent received PO cost</option><option value="2">Most recent received PO cost + shipping (landed cost)</option><option value="3">Average cost over time</option></select></div>{general.cost_calculation_method==='3'&&<React.Fragment><div style={{marginBottom:24,padding:'20px 24px',background:'rgba(255,255,255,.04)',borderRadius:12,border:'1px solid rgba(255,255,255,.08)'}}><label style={{display:'block',fontSize:14,fontWeight:600,marginBottom:6}}>Average Cost Period</label><p style={{fontSize:12,opacity:.5,marginBottom:10}}>How many days of purchase order data should be used to calculate the average?</p><select value={general.cost_avg_days} onChange={e=>setGeneral(g=>({...g,cost_avg_days:e.target.value}))} style={{width:'100%',padding:'10px 12px',borderRadius:8,border:'1px solid rgba(255,255,255,.15)',background:'rgba(255,255,255,.06)',color:'inherit',fontSize:14}}><option value="30">30 days</option><option value="60">60 days</option><option value="90">90 days</option></select></div><div style={{marginBottom:24,padding:'20px 24px',background:'rgba(255,255,255,.04)',borderRadius:12,border:'1px solid rgba(255,255,255,.08)'}}><label style={{display:'block',fontSize:14,fontWeight:600,marginBottom:6}}>Average Cost Type</label><p style={{fontSize:12,opacity:.5,marginBottom:10}}>Should the average include shipping costs (landed cost)?</p><select value={general.cost_avg_type} onChange={e=>setGeneral(g=>({...g,cost_avg_type:e.target.value}))} style={{width:'100%',padding:'10px 12px',borderRadius:8,border:'1px solid rgba(255,255,255,.15)',background:'rgba(255,255,255,.06)',color:'inherit',fontSize:14}}><option value="cost">Average cost only</option><option value="landed">Average landed cost (includes shipping)</option></select></div></React.Fragment>}<div style={{marginBottom:32,padding:'20px 24px',background:'rgba(255,255,255,.04)',borderRadius:12,border:'1px solid rgba(255,255,255,.08)'}}><label style={{display:'block',fontSize:14,fontWeight:600,marginBottom:6}}>Archive Sync with Shopify</label><p style={{fontSize:12,opacity:.5,marginBottom:10}}>When you archive an item in StockCentral, should it also be archived in Shopify?</p><select value={general.archive_sync} onChange={e=>setGeneral(g=>({...g,archive_sync:e.target.value}))} style={{width:'100%',padding:'10px 12px',borderRadius:8,border:'1px solid rgba(255,255,255,.15)',background:'rgba(255,255,255,.06)',color:'inherit',fontSize:14}}><option value="both">Two-way — archive in StockCentral also archives in Shopify</option><option value="one-way">One-way — only archive in StockCentral</option><option value="none">No sync — never archive in Shopify</option></select></div><button className="btn btn-primary" style={{padding:'12px 28px',fontSize:15}} disabled={savingGeneral} onClick={async()=>{setSavingGeneral(true);try{await api.put('/settings/general',general);toast.success('General settings saved');}catch(e){toast.error('Failed to save');}finally{setSavingGeneral(false);}}}>{savingGeneral?'Saving...':'Save General Settings'}</button></div>)}       {tab==='shopify'&&(         <div className="settings-section">           <h2 className="settings-section-title">General Settings</h2>           <div className="settings-group">             <label className="settings-label">Cost Update Mode</label>             <select value={general.cost_update_mode} onChange={e=>setGeneral(g=>({...g,cost_update_mode:e.target.value}))} className="settings-input">               <option value="auto">Auto — update cost when PO items are received</option>               <option value="manual">Manual — update cost manually</option>             </select>           </div>           <div className="settings-group">             <label className="settings-label">Cost Calculation Method</label>             <select value={general.cost_calculation_method} onChange={e=>setGeneral(g=>({...g,cost_calculation_method:e.target.value}))} className="settings-input">               <option value="1">Most recent received PO cost</option>               <option value="2">Most recent received PO cost + shipping (landed cost)</option>               <option value="3">Average cost over time</option>             </select>           </div>           {general.cost_calculation_method==='3'&&<React.Fragment>            <div className="settings-group">               <label className="settings-label">Average Cost Period</label>               <select value={general.cost_avg_days} onChange={e=>setGeneral(g=>({...g,cost_avg_days:e.target.value}))} className="settings-input">                 <option value="30">30 days</option>                 <option value="60">60 days</option>                 <option value="90">90 days</option>               </select>             </div>             <div className="settings-group">               <label className="settings-label">Average Cost Type</label>               <select value={general.cost_avg_type} onChange={e=>setGeneral(g=>({...g,cost_avg_type:e.target.value}))} className="settings-input">                 <option value="cost">Average cost only</option>                 <option value="landed">Average landed cost (includes shipping)</option>               </select>             </div></React.Fragment>}<div className="settings-group">             <label className="settings-label">Archive Sync with Shopify</label>             <select value={general.archive_sync} onChange={e=>setGeneral(g=>({...g,archive_sync:e.target.value}))} className="settings-input">               <option value="both">Two-way — archive in StockCentral also archives in Shopify</option>               <option value="one-way">One-way — only archive in StockCentral</option>               <option value="none">No sync — never archive in Shopify</option>             </select>           </div>           <button className="btn btn-primary" disabled={savingGeneral} onClick={async()=>{setSavingGeneral(true);try{await api.put('/settings/general',general);toast.success('General settings saved');}catch(e){toast.error('Failed to save');}finally{setSavingGeneral(false);}}}>{savingGeneral?'Saving...':'Save General Settings'}</button>         </div>       )}
{tab==='shopify'&&(
