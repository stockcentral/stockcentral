...
export default function Orders() {
  ...
  const openOrder = async (order) => {
            ...
            setDetail(null); setNotes([]); setSelectedItems([]);
  ...
  // Handle Shopify cancellation — marks order cancelled and updates list instantly
  const markCancelled = async (orderId) => {
            ...
            if (cancelledStatus) {
                        await updateStatus(orderId, cancelledStatus.id, cancelledStatus.name);
            } else {
                        // No cancelled status exists — update the raw status field
              try {
                            await api.put(`/orders/${orderId}/status`, { status: 'cancelled' });
                            setOrders(prev => prev.map(o => o.id === orderId ? { ...o, status_name: 'Cancelled', status_color: '#ef4444' } : o));
                            if (selectedOrder?.id === orderId) setSelectedOrder(prev => ({ ...prev, status_name: 'Cancelled', status_color: '#ef4444' }));
...
{lineItems.length>0 && <span style={{opacity:.5}}> — {preview}{lineItems.length>2?'...':''}</span>}
                    ...
                    <td onClick={e=>e.stopPropagation()}>
                        ...
                        onChange={e=>{const s=statuses.find(x=>x.id===e.target.value); if(s) updateStatus(order.id,s.id,s.name);}}
...
{/* Header — big customer name + order totals */}
            <div className="modal-header" style={{flexShrink:0,paddingBottom:0}}>
              <div style={{flex:1}}>
{/* Order # + status selector */}
                <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:6,flexWrap:'wrap'}}>
                  <span style={{fontSize:13,opacity:.5,fontWeight:500}}>Order #{selectedOrder.order_number}</span>
                  <select
                    value={selectedOrder.custom_status_id||''}
                    onChange={e=>{const s=statuses.find(x=>x.id===e.target.value); if(s) updateStatus(selectedOrder.id,s.id,s.name);}}
                    style={{padding:'4px 10px',borderRadius:12,border:'none',cursor:'pointer',fontWeight:600,fontSize:12,background:`${getStatusInfo(selectedOrder).color}22`,color:getStatusInfo(selectedOrder).color,outline:'none'}}>
                    <option value="" disabled>{getStatusInfo(selectedOrder).name}</option>
{statuses.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}
      </select>
{/* Cancel button */}
                  <button onClick={()=>markCancelled(selectedOrder.id)}
                    style={{marginLeft:'auto',fontSize:11,padding:'3px 10px',borderRadius:6,border:'1px solid rgba(239,68,68,0.4)',background:'none',cursor:'pointer',color:'#ef4444'}}>
                    Mark Cancelled
                          </button>
                          </div>
...
{selectedOrder.customer_name||'Unknown Customer'}
                ...
                <div style={{fontSize:13,opacity:.55,marginTop:3}}>{selectedOrder.customer_email||''}</div>

{/* Order totals row */}
                <div style={{display:'flex',gap:20,marginTop:12,paddingTop:12,borderTop:'1px solid rgba(255,255,255,0.08)',flexWrap:'wrap'}}>
{[
      { label:'Subtotal', value: selectedOrder.subtotal_price || selectedOrder.total_price },
      { label:'Shipping', value: selectedOrder.total_shipping_price || 0 },
      { label:'Tax', value: selectedOrder.total_tax || 0 },
      { label:'Order Total', value: selectedOrder.total_price, highlight: true },
                        ].map(({ label, value, highlight }) => (
                                                  <div key={label} style={{textAlign:'left'}}>
                      <div style={{fontSize:11,opacity:.45,textTransform:'uppercase',letterSpacing:'.04em'}}>{label}</div>
                      <div style={{fontSize: highlight?18:15, fontWeight: highlight?700:600, color: highlight?'var(--text-primary)':'inherit', marginTop:2}}>
                        ${parseFloat(value||0).toFixed(2)}
</div>
      </div>
                  ))}
                  <div style={{textAlign:'left'}}>
                    <div style={{fontSize:11,opacity:.45,textTransform:'uppercase',letterSpacing:'.04em'}}>Date</div>
                    <div style={{fontSize:14,fontWeight:500,marginTop:2}}>{new Date(selectedOrder.created_at).toLocaleDateString()}</div>
                        </div>
                        </div>
                        </div>
              <button className="modal-close" style={{alignSelf:'flex-start'}} onClick={closeOrder}><X size={18}/></button>
                        </div>

{/* Body */}
            <div className="modal-body" style={{flex:1,overflowY:'auto',paddingTop:16}}>
{loadingDetail ? (
...
{(detail.line_items_enriched||[]).length===0 && (
                          <div style={{opacity:.5,fontSize:13,padding:16}}>No line items found.</div>
                  )}
...
{(detail.line_items_enriched||[]).map((item,idx) => {
                          ...
                          <div style={{flex:1}}>
                            ...
                            <div style={{fontSize:12,opacity:.5,marginTop:2}}>
                              SKU: {inv?.sku||item.sku||'N/A'} &nbsp;·&nbsp; Qty: <strong>{item.quantity}</strong>
{item.price && <span> &nbsp;·&nbsp; ${parseFloat(item.price).toFixed(2)}/ea</span>}
{!inv && <span style={{marginLeft:8,fontStyle:'italic',color:'#f59e0b'}}>⚠ Not in inventory</span>}
      </div>
      </div>
{inv && (
...
{item.bom&&item.bom.length>0&&(
                            ...
                            <div style={{fontSize:11,fontWeight:600,textTransform:'uppercase',opacity:.4,marginBottom:8,display:'flex',alignItems:'center',gap:6}}>
                              <Factory size={11}/> Bill of Materials
      </div>
...
{available}{available<0&&<AlertTriangle size={11} style={{marginLeft:3,display:'inline',verticalAlign:'middle'}}/>}
      </td>
      </tr>
                                  );
})}
