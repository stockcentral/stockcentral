    ...
    <div className="page-container">
      ...
      <div style={{display:'flex',gap:10,marginBottom:16,flexWrap:'wrap',alignItems:'center'}}>
        ...
        <select value={filterCategory} onChange={e=>setFilterCategory(e.target.value)}
          style={{padding:'8px 12px',borderRadius:8,border:'1px solid rgba(255,255,255,.12)',background:'var(--bg-secondary,#1a1a2e)',color:'inherit',fontSize:13,minWidth:130}}>
          <option value="">All Categories</option>
          {categories.map(c=><option key={c} value={c}>{c}</option>)}
        </select>
        <div style={{display:'flex',alignItems:'center',gap:6}}>
          <span style={{fontSize:12,opacity:.5,whiteSpace:'nowrap'}}>Price</span>
          <input value={filterPriceMin} onChange={e=>setFilterPriceMin(e.target.value)} placeholder="Min" type="number"
            style={{width:70,padding:'7px 10px',borderRadius:8,border:'1px solid rgba(255,255,255,.12)',background:'var(--bg-secondary,#1a1a2e)',color:'inherit',fontSize:13}}/>
          <span style={{opacity:.4}}>—</span>
          <input value={filterPriceMax} onChange={e=>setFilterPriceMax(e.target.value)} placeholder="Max" type="number"
            style={{width:70,padding:'7px 10px',borderRadius:8,border:'1px solid rgba(255,255,255,.12)',background:'var(--bg-secondary,#1a1a2e)',color:'inherit',fontSize:13}}/>
        </div>
        {hasFilters && (
          <button onClick={()=>{setSearch('');setFilterCategory('');setFilterPriceMin('');setFilterPriceMax('');}}
      ...
      {loading ? <div className="loading">Loading...</div> : (
        <div className="table-container">
          <table className="data-table">
            ...
            <tbody>
              {filtered.map(item=>(
                <tr key={item.id} className={`hover-row${item.quantity<=item.low_stock_threshold?' low-stock-row':''}`}
                  onClick={()=>openEdit(item)} style={{cursor:'pointer'}}>
                  <td onClick={e=>e.stopPropagation()}>
                    <input type="checkbox" checked={selectedIds.includes(item.id)} onChange={e=>toggleSelect(item.id,e)}/>
                  </td>
                  <td>{item.photo_url
                    ? <img src={item.photo_url} alt="" style={{width:36,height:36,borderRadius:6,objectFit:'cover'}}/>
                    : <div style={{width:36,height:36,borderRadius:6,background:'rgba(255,255,255,.06)',display:'flex',alignItems:'center',justifyContent:'center'}}><Package size={18} style={{opacity:.3}}/></div>}
                  </td>
          ...
          <div className="modal large-modal">
            <div className="modal-header">
              ...
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                {editing && (
                  <button className="btn btn-secondary" style={{fontSize:12,padding:'5px 12px',display:'flex',alignItems:'center',gap:5}}
                    onClick={handlePushToShopify} disabled={pushingShopify}>
                    {pushingShopify ? <RefreshCw size={13} style={{animation:'spin 1s linear infinite'}}/> : <ArrowUpRight size={13}/>}
            ...
            <div className="modal-body">
              ...
              <div style={{display:'flex',gap:16,alignItems:'flex-start',marginBottom:16}}>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:8}}>
                  ...
                  <button className="btn btn-secondary" style={{fontSize:12}} onClick={()=>photoRef.current.click()}>
                    <Upload size={13}/> {photoPreview ? 'Change Photo' : 'Upload Photo'}
                  </button>
                  {photoPreview && (
                    <button className="btn btn-ghost" style={{fontSize:12}} onClick={()=>{setPhotoPreview(null);setPhotoFile(null);setForm(f=>({...f,photo_url:''}));}}>
                      Remove Photo
                    </button>
                  )}
              ...
              <div style={{display:'flex',alignItems:'center',gap:10,margin:'12px 0',padding:'10px 12px',background:'rgba(139,92,246,0.1)',borderRadius:8,border:'1px solid rgba(139,92,246,0.3)'}}>
                ...
                <label htmlFor="is_manufactured" style={{cursor:'pointer',display:'flex',alignItems:'center',gap:6,fontWeight:500}}>
                  ...
                  <span style={{fontSize:12,opacity:.6,fontWeight:400}}>— has a Bill of Materials</span>
                </label>
              </div>
              ...
              {editing && form.shopify_product_id && (
                <div style={{marginTop:16,padding:'10px 12px',background:'rgba(16,185,129,.08)',borderRadius:8,border:'1px solid rgba(16,185,129,.2)',fontSize:12}}>
                  <span style={{color:'#10b981',fontWeight:600}}>Shopify linked</span>
                  <span style={{opacity:.5,marginLeft:8}}>Product ID: {form.shopify_product_id} · Variant: {form.shopify_variant_id}</span>
                </div>
              )}
            </div>
