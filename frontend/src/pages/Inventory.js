// Inventory Page - InputField defined OUTSIDE component to prevent remount on keystroke
import React, { useState, useEffect, useRef } from 'react';
import api from '../utils/api';
import toast from 'react-hot-toast';
import { Plus, Search, Edit, Trash2, Upload, Download, Package, AlertTriangle, X } from 'lucide-react';
...
// CRITICAL: Defined outside Inventory so React does not remount on every state change
...
export default function Inventory() {
  ...
  const attemptClose = () => {
          if (isDirty) {
                    if (window.confirm('Save as draft or discard changes?')) { handleSave(true); }
                    else { setShowModal(false); setIsDirty(false); }
  ...
  // Helper to pass standard props to InputField
...
{loading ? <div className="loading">Loading...</div> : (
         <div className="table-container">
              ...
              <th>Photo</th><th>SKU</th><th>Name</th><th>Category</th><th>Stock</th><th>Cost</th><th>Price</th><th>Actions</th>
    </tr></thead>
                <tbody>
{filtered.map(item=>(
...
{filtered.length===0&&<tr><td colSpan={9} style={{textAlign:'center',padding:32,opacity:.5}}>No items found</td></tr>}
    </tbody>
    </table>
    </div>
      )}
...
{showModal&&(
          ...
          <div className="modal large-modal">
                <div className="modal-header"><h2>{editing?'Edit Item':'Add New Item'}</h2><button className="modal-close" onClick={attemptClose}><X size={18}/></button></div>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={()=>handleSave(false)}>{editing?'Save Changes':'Add Item'}</button>
    </div>
    </div>
    </div>
      )}
