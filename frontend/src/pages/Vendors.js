...
// Reusable quick-add popup for use in dropdowns across the app
export function AddVendorPopup({ onCreated, onClose }) {
  ...
  const handleSave = async () => {
        ...
        try {
                ...
                onCreated(res.data);
        } catch(e) { toast.error(e.response?.data?.error || 'Failed to add vendor'); }
...
// Vendor select dropdown with built-in "Add New Vendor" option
export function VendorSelect({ value, onChange, name, className }) {
  ...
  const handleCreated = (vendor) => {
        setVendors(v=>[...v, vendor]);
        onChange({ target: { name, value: vendor.id } });
        setShowAdd(false);
  };
...
export default function Vendors() {
  ...
  const handleCSVImport = async (e) => {
        ...
        try {
                const r = await api.post('/vendors/import', { vendors, source: 'csv' });
                toast.success(r.data.message); fetchVendors(); setShowImport(false);
...
{filtered.map(v=>(
                  ...
                  <td>
                      <button className="btn-icon" onClick={()=>openEdit(v)}><Edit size={14}/></button>
