...
// Seed endpoint — inserts sample orders + inventory for testing
app.post('/seed', async (req, res) => {
    try {
          const { secret } = req.body;
          if (secret !== 'seed-stockcentral-2024') return res.status(403).json({ error: 'Forbidden' });

      // Get the default Paid status
      ...
      const orders = [
        {
                  shopify_order_id: 'SAMPLE-1001',
                  order_number: '1001',
                  customer_name: 'James Rivera',
                  customer_email: 'james.rivera@example.com',
                  total_price: 429.97,
                  line_items: [
        ...
        {
                  shopify_order_id: 'SAMPLE-1003',
                  order_number: '1003',
                  customer_name: 'Marcus Thompson',
                  customer_email: 'm.thompson@example.com',
                  total_price: 179.94,
                  line_items: [
      ...
      let inserted = 0;
          for (const order of orders) {
                  await pool.query(
                            `INSERT INTO shopify_orders (shopify_order_id, order_number, customer_name, customer_email, total_price, line_items, status, custom_status_id)
                                     VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
                                              ON CONFLICT (shopify_order_id) DO NOTHING`,
                            [order.shopify_order_id, order.order_number, order.customer_name, order.customer_email,
                                      order.total_price, JSON.stringify(order.line_items), 'paid_unfulfilled', paidStatusId]
                          );
                  inserted++;
          }
