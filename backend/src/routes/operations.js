...
bomRouter.get('/:productId', async (req, res) => {
    try {
          const result = await pool.query('SELECT b.*, ii.sku as component_sku, ii.name as component_name, ii.cost as component_cost, ii.quantity as component_stock FROM bom b JOIN inventory_items ii ON b.component_id = ii.id WHERE b.finished_product_id=$1', [req.params.productId]);
          res.json(result.rows);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
bomRouter.post('/', async (req, res) => {
    try {
          ...
          const result = await pool.query('INSERT INTO bom (finished_product_id, component_id, quantity, notes) VALUES ($1,$2,$3,$4) RETURNING *', [toUUID(finished_product_id), toUUID(component_id), quantity||1, notes||null]);
          res.json(result.rows[0]);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
