  ...
  try {
    const result = await pool.query(`
      SELECT po.*, v.name as vendor_name,
        (SELECT COALESCE(SUM(pi.quantity_received),0) FROM po_items pi WHERE pi.po_id=po.id) as total_received,
        (SELECT COALESCE(SUM(pi.quantity_ordered),0) FROM po_items pi WHERE pi.po_id=po.id) as total_ordered
...
router.post('/', async (req, res) => {
  try {
    ...
    const po = await pool.query(
      `INSERT INTO purchase_orders (po_number, vendor_id, notes, expected_date, total, status)
       VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
      [genPONumber(), vendor_id, notes||null, expected_date||null, total, status||'pending']
    );
    for (const item of (items||[])) {
      if (!item.inventory_item_id) continue;
      await pool.query(
        `INSERT INTO po_items (po_id, inventory_item_id, quantity_ordered, unit_cost, quantity_received, rejected_quantity)
         VALUES ($1,$2,$3,$4,0,0)`,
...
router.put('/:id/receive-item', async (req, res) => {
  try {
    ...
    const item = await pool.query(
      `UPDATE po_items SET quantity_received = quantity_received + $1 WHERE id=$2 RETURNING *`,
      [qty, itemId]
    );
    ...
    } else if (item.rows[0].quantity_received > 0 && po.rows[0].status === 'pending') {
      await pool.query(`UPDATE purchase_orders SET status='partial', updated_at=NOW() WHERE id=$1`, [req.params.id]);
    }
    await updateInventoryCostFromPO(item.rows[0].inventory_item_id, item.rows[0].unit_cost, po.rows[0]?.shipping_cost);
