...
router.get('/general', async (req, res) => {   try {     const keys = ['cost_update_mode','cost_calculation_method','cost_avg_days','cost_avg_type','archive_sync','shopify_push_mode'];     const result = await pool.query(`SELECT key, value FROM settings WHERE key = ANY($1)`, [keys]);     const map = {};     result.rows.forEach(r => { map[r.key] = r.value; });     res.json({ cost_update_mode: map.cost_update_mode||'auto', cost_calculation_method: map.cost_calculation_method||'1', cost_avg_days: map.cost_avg_days||'30', cost_avg_type: map.cost_avg_type||'cost', archive_sync: map.archive_sync||'both' });   } catch(err) { res.status(500).json({ error: err.message }); } });  router.put('/general', async (req, res) => {   try {     const { cost_update_mode, cost_calculation_method, cost_avg_days, cost_avg_type, archive_sync } = req.body;     const updates = { cost_update_mode, cost_calculation_method, cost_avg_days, cost_avg_type, archive_sync };     for (const [key, value] of Object.entries(updates)) {       if (value !== undefined) await pool.query(`INSERT INTO settings (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value = $2`, [key, value]);     }     res.json({ success: true });   } catch(err) { res.status(500).json({ error: err.message }); } });
